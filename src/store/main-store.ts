...
export class MainStore {
    ...
    setOpenModal = (value: boolean, reasonCode: number) => {
        ...
        if (reasonCode == 2) {
            this.modalText = "Twitter account already linked to another Wallet. Please use another twitter account."
        }
        if (reasonCode == 3) {
            this.modalText = "Something went wrong when trying to contact Twitter. Please try again."
        }
        if (reasonCode == 4) {
            this.modalText = "You didn't approve the link between our application and your twitter account!"
        }
